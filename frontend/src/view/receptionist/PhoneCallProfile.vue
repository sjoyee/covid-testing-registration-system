<template>
  <div>
    ...
    <div class="ma-4 text-h5 font-weight-bold pa-4">
      Modify Active Booking Through Phone Call
    </div>
    <v-row v-if="successFound">
      <v-card flat tile class="ma-4" v-if="bookingDetails">
        <div class="ma-4 text-subtitle-1 px-4">
          Only display active booking.
        </div>
    ...
    <div v-else>
      <div class="ma-4 text-subtitle-1 px-4">
        Please request booking ID or PIN code from the customer to perform
        verification.
      </div>
      <v-col md="6">
        <v-form ref="form">
          <v-card class="ma-4 pa-4" tile>
            <v-card-subtitle>Verify Booking ID.</v-card-subtitle>
            <v-row class="pa-4">
              <v-col>
                <v-text-field
                  v-model="bookingId"
          ...
          <v-card class="ma-4 pa-4" tile>
            <v-card-subtitle>Verify PIN Code.</v-card-subtitle>
            <v-row class="pa-4">
              <v-col>
                <v-text-field v-model="smsPin" label="PIN Code"></v-text-field>
              </v-col>
            </v-row>
            <v-card-actions class="pa-4">
              <v-spacer></v-spacer>
              <v-btn @click="getActiveBookingByPin">CONFIRM</v-btn>
            </v-card-actions>
          </v-card>
        </v-form>
      </v-col>
  ...
  methods: {
    ...
    async getActiveBookingById() {
      try {
        ...
        console.log("Fail to get active booking with the provided id.");
      }
      this.$refs.form.reset();
    },

    async getActiveBookingByPin() {
      try {
        const response = await axios.get(
          `/${this.$route.params.id}/booking/active-booking-by-pin?smsPin=${this.smsPin}`
        );
        this.bookingDetails = response.data;
        this.successFound = true;
      } catch {
        console.log("Fail to get active booking with the provided pin.");
      }
      this.$refs.form.reset();
    },
